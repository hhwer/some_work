...
'''modified by huang: debug for mp'''
sys.stderr = open("log/" +"log.out","a")
...
#             print('cp',cp,'lengthscales',lengthscales)
        ...
        self.initial_hyperparams['length scales'] = copy.copy(self.lengthscales)
    ...
    def OptIter(self,pause=0):
        ...
        '''modified by huang'''
        if x_next in self.X_obs[-5:]:
            print('x_next=', x_next,  ' is used to update GP recently, we changed it near to maximun one')
            np.random.seed(len(self.X_obs))
            x_next = self.x_best - np.random.uniform(0, self.lengthscales)
    ...
    def acquire(self, alpha=1.):
        ...
        try:
            ...
            if(self.multiprocessingQ): # multi-processing to speed search
                ...
                """modified by huang change bounds"""
                #print('v0best=',v0best)
                #print('v0s=',v0s)
                #print('x0s=',x0s)
                #print('bounds_before=',iter_bounds)
                #print('relative_bounds=',relative_bounds)
                #iter_bounds[0][0] = max(iter_bounds[0][0], self.bounds[0][0])
                #iter_bounds[0][1] = min(iter_bounds[0][1], self.bounds[0][1])
                #iter_bounds[0][0] = max(iter_bounds[0][0], self.bounds[0][0])
                #iter_bounds[0][1] = min(iter_bounds[0][1], self.bounds[0][1])
                #relative_bounds[0][0] = max(relative_bounds[0][0]+x_start, self.bounds[0][0]) - x_start
                #relative_bounds[0][1] = min(relative_bounds[0][1]+x_start, self.bounds[0][1]) - x_start
                #print('bounds_modified=',iter_bounds)
                #print('relative_bounds_modified=',relative_bounds)
                
                
                
                if basinhoppingQ:
                    # use basinhopping
                    print('use basinhopping add by huang')
                    ...
                    # use minimize
                    print('use minimize add by huang')
                    ...
                    res = parallelminimize(aqfcn,x0s,fargs,mkwargs,v0best,relative_bounds=relative_bounds, modified='huang', bounds=self.bounds)
                    #res = parallelminimize(aqfcn,x0s,fargs,mkwargs,v0best,relative_bounds=relative_bounds, modified='huang', )
...
def negUCB(x_new, model, ndim, nsteps, nu = 1., delta = 1.):
    ...
    '''modified by huang: in mp print out and err in log file'''
    #sys.stdout = open("log/" +"log.out","a")
    #sys.stderr = open("log/" +"log.out","a")
    #print('x_new',x_new)
    #sys.stdout = open("log/"+str(os.getpid()) +".out","w")
    #sys.stderr = open("log/"+str(os.getpid()) +".out","w")
    #print('nsteps',nsteps,type(nsteps))
    #print('ymean,y_var:',y_mean,y_var)
    #print('nu,delta,nsteps',nu,delta,nsteps)
    #print('nsteps',nsteps,type(nsteps),nsteps.shape)
    #sys.stdout.flush() 
    
    
    '''modified by huang: prevent error for sqrt(var<0)'''
    if y_var < 10**(-10):
        y_var = 0
        return -y_mean
    ...
    #print('-GPUCB:,',-GPUCB)
    #print('-GPUCB:,',-GPUCB,'ymean,y_var:',y_mean,y_var,'/n')
    #sys.stdout.flush()
    return -GPUCB
