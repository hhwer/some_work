...
try:
    ...
    def parallelminimize(f,x0s,fargs,margs,v0best=None,relative_bounds=None, modified=None, bounds=None):
        ...
        # modified is set to huang means this function will do something different than before. Otherwise, set modified 
...
try:
    ...
    #    res = np.array(pool.map(l, range(10)))
        ...
        for b in range(nbatch):
            ...
            while True:
                try:
                    ...
                    for i in range(ilow, ihigh):
                        '''modified by huang: print log in subprocess'''
                        p = mp.Process(
                                target=worker,
                                args=args[i]+tuple([queues[i-ilow]]))
        ...
        '''modified by huang, prevent from nan'''
        #if np.isnan(res[nkeep-1,-1]):
        #
